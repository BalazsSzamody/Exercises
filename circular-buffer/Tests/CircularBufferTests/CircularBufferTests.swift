...
class CircularBufferTests: XCTestCase {
    ...
    func testLongBuffer() {
        var buffer = CircularBuffer<Int>(capacity: 500)
        (1 ... 500).forEach { try? buffer.write($0) }
        (501 ... 510).forEach { buffer.overwrite($0) }
        for i in 1 ... 490 {
            _ = try? buffer.read()
        }
        (501 ... 510).forEach { XCTAssertEqual($0, try? buffer.read())}
    ...
    func testPerf() {
        let max = 10_000
        measure {
            var cb = CircularBuffer< Int >( capacity: max )
            for i in 1...max { try! cb.write( i ) }
            for _ in 1...max { _ = try! cb.read() }
            for i in 1...max { cb.overwrite( i ) }
            cb.clear()
        }
    }
